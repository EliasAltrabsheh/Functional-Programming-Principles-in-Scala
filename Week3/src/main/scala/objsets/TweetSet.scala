 ...
 * [1] http://en.wikipedia.org/wiki/Binary_search_tree
  *
  *
  *he tweets are indexed by their text bodies:
...
/**
  ...
  *
  * when do you need to create an abstract class
  * 1) You want to create a base class that requires constructor arguments.
  * 2)the code will be called from Java code.
  *
  *
  * TweetSet with two concrete subclasses,
  * 1) Empty which represents an empty set,
  * 2) NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet), which represents a non-empty set as a binary tree rooted at elem.
  */
...
abstract class TweetSet {
  ...
  /**
   * This is a helper method for `filter` that propagate the accumulated tweets.
   */
...
class Empty extends TweetSet {
  // This case you have an empty set so we return the accumulator that is the set given
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    }  // Helping inner function to return largest Tweet
...
// Trait defined in scala can be used as abstract class but when it has more then one superclass we can use traits, it is normally used as
// an interface that is defined between multiple classes(base) so we can inherit also methods that have fields and abstract methods
